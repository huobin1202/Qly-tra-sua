...
public class ThemDonHangView extends JDialog {
            ...
            try (PreparedStatement checkPs = conn.prepareStatement("SELECT MaKH FROM khachhang WHERE SDT = ?")) {
                ...
                try (ResultSet rs = checkPs.executeQuery()) {
                    if (rs.next()) {
                        ...
                        try (PreparedStatement ps = conn.prepareStatement(insertKhSql,
    ...
    private void persistOrderDetails(Connection conn) throws SQLException {
        // Đảm bảo đơn hàng đã tồn tại trước khi lưu chi tiết
        if (currentOrder.getMaDon() <= 0) {
            throw new SQLException("Đơn hàng chưa được tạo. Vui lòng tạo đơn hàng trước.");
        }
        ...
        try (PreparedStatement ps = conn.prepareStatement(updateOrderSql)) {
            ...
            if (rowsUpdated == 0) {
                throw new SQLException("Không tìm thấy đơn hàng với MaDon = " + currentOrder.getMaDon());
            }
        }
    ...
    private void initializeComponents() {
        ...
        giamGiaSpinner = new JSpinner(new SpinnerNumberModel(0, 0, 100, 1)); // Không giới hạn, tối đa 100%
    ...
    private void loadNhanVienName() {
        try (Connection conn = DBUtil.getConnection()) {
            ...
            JOptionPane.showMessageDialog(this, "Lỗi tải thông tin nhân viên: " + e.getMessage(), "Lỗi",
    ...
    private void loadKhachHang() {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            JOptionPane.showMessageDialog(this, "Lỗi tải danh sách khách hàng: " + e.getMessage(), "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void timKiemKhachHangTheoSDT() {
        ...
        if (!sdt.matches("\\d+")) {
            JOptionPane.showMessageDialog(this, "Số điện thoại chỉ được chứa chữ số!", "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
        if (sdt.length() > 11) {
            JOptionPane.showMessageDialog(this, "Số điện thoại tối đa 11 số!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    JOptionPane.showMessageDialog(this, "Đã tìm thấy khách hàng!", "Thành công",
                            JOptionPane.INFORMATION_MESSAGE);
                } else {
                    // Không tìm thấy khách hàng - khách hàng mới
                    JOptionPane.showMessageDialog(this, "Khách hàng mới. Vui lòng nhập đầy đủ thông tin!", "Thông báo",
                            JOptionPane.INFORMATION_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, "Lỗi tìm kiếm khách hàng: " + e.getMessage(), "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void loadCategories(JPanel categoryPanel) {
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            JOptionPane.showMessageDialog(this, "Lỗi tải danh mục: " + e.getMessage(), "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void setupEventHandlers() {
        ...
        capNhatButton.addActionListener(e -> {
            if (!isEditable()) {
                JOptionPane.showMessageDialog(this, "Đơn đã thanh toán hoặc bị hủy, không thể lưu/sửa!",
                        "Không thể thao tác", JOptionPane.WARNING_MESSAGE);
                return;
            }
            saveOrder();
        });
        ...
        thanhToanButton.addActionListener(e -> {
            if (!isEditable()) {
                JOptionPane.showMessageDialog(this, "Đơn đã thanh toán hoặc bị hủy, không thể thanh toán!",
                        "Không thể thao tác", JOptionPane.WARNING_MESSAGE);
                return;
            }
            processPayment();
        ...
        huyHoaDonButton.addActionListener(e -> {
            if (!isEditable()) {
                JOptionPane.showMessageDialog(this, "Đơn đã thanh toán hoặc đã hủy, không thể hủy!",
                        "Không thể thao tác", JOptionPane.WARNING_MESSAGE);
                return;
            }
            cancelOrder();
    ...
    private void addProductToOrder(MonDTO product) {
        if (!isEditable()) {
            JOptionPane.showMessageDialog(this, "Đơn đã thanh toán hoặc bị hủy, không thể thêm món!",
                    "Không thể thao tác", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private String kiemTraNguyenLieu(int maMon, int soLuong, int maMonCheck, String tenTopping) {
        ...
        // Đếm nguyên liệu từ các món đã có trong đơn hàng (bao gồm cả topping của
        // chúng)
        for (ChiTietDonHangDTO item : orderedItems) {
    ...
    private JPanel createOrderedItemPanel(ChiTietDonHangDTO item, int index) {
        ...
        minusButton.addActionListener(e -> {
            if (!isEditable()) {
                JOptionPane.showMessageDialog(this, "Đơn đã thanh toán hoặc bị hủy, không thể chỉnh số lượng!",
                        "Không thể thao tác", JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    private void updateOrderSummary() {
        ...
        // Kiểm tra đơn đầu tiên của khách hàng
        boolean isFirstOrder = false;
        Integer maKH = currentOrder.getMaKH();
        if (maKH != null && maKH > 0) {
            try (Connection conn = DBUtil.getConnection()) {
                try (PreparedStatement ps = conn.prepareStatement(
                        "SELECT COUNT(*) FROM donhang WHERE MaKH = ? AND TrangThai = 'dathanhtoan'")) {
                    ps.setInt(1, maKH);
                    try (ResultSet rs = ps.executeQuery()) {
        ...
        if (isFirstOrder && totalLy > 0) {
            ...
            // Tự động tính giảm giá đề xuất theo điểm tích lũy hiện có của khách hàng (tối
            // đa 30%)
            try {
        ...
        // Chỉ tự động cập nhật nếu người dùng chưa chỉnh tay hoặc giá trị đề xuất khác
    ...
    private int getCategoryIdByIndex(int index) {
        try (Connection conn = DBUtil.getConnection()) {
            ...
            JOptionPane.showMessageDialog(this, "Lỗi lấy danh mục: " + e.getMessage(), "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void saveOrder() {
        ...
        if (orderedItems.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng thêm ít nhất một sản phẩm vào đơn hàng!", "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try (Connection conn = DBUtil.getConnection()) {
                    ...
                    try (ResultSet rs = checkPs.executeQuery()) {
                        if (rs.next()) {
                            ...
                            JOptionPane.showMessageDialog(this,
                                    "Số điện thoại này đã được sử dụng bởi khách hàng khác!", "Lỗi",
                                    JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
            ...
            if (currentOrder.getMaDon() <= 0) {
                conn.rollback();
                JOptionPane.showMessageDialog(this, "Lỗi: Không thể tạo đơn hàng!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }

            // Cập nhật MaKH vào đơn hàng nếu có
            if (maKH > 0) {
                currentOrder.setMaKH(maKH);
            }
            ...
            try (PreparedStatement ps = conn.prepareStatement(updateOrderSql)) {
                ...
                if (rowsUpdated == 0) {
                    conn.rollback();
                    JOptionPane.showMessageDialog(this,
                            "Lỗi: Không tìm thấy đơn hàng với ID = " + currentOrder.getMaDon(), "Lỗi",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
            ...
            JOptionPane.showMessageDialog(this, "Lỗi lưu đơn hàng: " + e.getMessage(), "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void processPayment() {
        ...
        if (orderedItems.isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    "Vui lòng thêm ít nhất một sản phẩm vào đơn hàng!.", "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (result == JOptionPane.YES_OPTION) {
            ...
            try (Connection conn = DBUtil.getConnection()) {
                ...
                JOptionPane.showMessageDialog(this, "Lỗi khi lưu đơn trước thanh toán: " + ex.getMessage(), "Lỗi",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            if (donHangDAO.capNhatTrangThaiDonHang(currentOrder.getMaDon(), "dathanhtoan")) {
                ...
                try (Connection conn = DBUtil.getConnection()) {
                    ...
                    if (maKH != null && maKH > 0) {
                        ...
                        try (PreparedStatement ps = conn
                                .prepareStatement("SELECT DiemTichLuy FROM khachhang WHERE MaKH=?")) {
                            ps.setInt(1, maKH);
                            try (ResultSet rs = ps.executeQuery()) {
                                if (rs.next())
                        ...
                        try (PreparedStatement ps = conn
                                .prepareStatement("UPDATE khachhang SET DiemTichLuy=? WHERE MaKH=?")) {
                ...
                JOptionPane.showMessageDialog(this, "Thanh toán thành công!\nNguyên liệu trong kho đã được cập nhật.",
                        "Thành công", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this,
                        "Lỗi thanh toán! Vui lòng kiểm tra lại số lượng nguyên liệu trong kho.", "Lỗi",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void printInvoice() {
        try {
            ...
            if (currentOrder.getMaDon() <= 0) {
                JOptionPane.showMessageDialog(this, "Vui lòng lưu đơn hàng trước khi in hóa đơn!", "Thông báo", JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    private void cancelOrder() {
        ...
        if (result == JOptionPane.YES_OPTION) {
            try (Connection conn = DBUtil.getConnection()) {
                ...
                JOptionPane.showMessageDialog(this, "Hủy đơn hàng thành công!", "Thành công",
                        JOptionPane.INFORMATION_MESSAGE);
            } catch (SQLException e) {
                JOptionPane.showMessageDialog(this, "Lỗi hủy đơn hàng: " + e.getMessage(), "Lỗi",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
